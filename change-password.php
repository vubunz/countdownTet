...
if (isset($_POST["submit"])) {
    ...
    if ($password == "") {
        ...
        $sql = "SELECT * FROM `player` WHERE `username` = '" . $_SESSION['username'] . "' AND `password` = '$password'";
        ...
        if ($num_rows == 0) {
            ...
            $sql = "UPDATE `player` SET `password` = '$new_password' WHERE `username` = '" . $_SESSION['username'] . "'";
            $query = mysqli_query($conn, $sql);
